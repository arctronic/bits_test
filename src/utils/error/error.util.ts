...
export const handleLibraryError = (error: any, res: Response) => {
  ...
  logger.error(JSON.stringify(errorResponseObject.logMessage));
  console.error(errorResponseObject.logMessage);

  return res.status(errorResponseObject.statusCode).json({
    status: errorResponseObject.statusCode,
    message: errorResponseObject.responseMessage
  });
};
